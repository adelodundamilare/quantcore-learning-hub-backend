...
def get_all_stock_option_categories(
    db: Session = Depends(deps.get_db),
    context: UserContext = Depends(deps.get_current_user_with_context)
):
    categories = stock_options_service.get_all_categories(db)
    return APIResponse(message="Stock option categories retrieved successfully", data=categories)

@router.get("/options/categories/{category_id}", response_model=APIResponse[StockCategorySchema])
def get_stock_option_category(
    category_id: int,
    db: Session = Depends(deps.get_db),
    context: UserContext = Depends(deps.get_current_user_with_context)
):
    category = stock_options_service.get_category_by_id(db, category_id=category_id)
    if not category:
        raise HTTPException(
